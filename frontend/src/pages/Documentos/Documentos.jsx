...
import { FileText, Upload, Search, Trash2, CheckCircle, X,
         Download, Eye, Pencil } from 'lucide-react'
...
function EditModal({ doc, clientes, onSave, onClose }) {
  ...
  useEffect(() => {
    ...
    clientesService.getProcedimientos(cliId).then(({ data }) => setProcs(data)).catch(() => setProcs([]))
  }, [cliId])
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: '#000a', zIndex: 1000,
      ...
      <div style={{ ...card({ width: 420, padding: 28 }), position: 'relative' }}>
        ...
        <label style={{ color: C.textS, fontSize: 12 }}>{t('documentos.tipoDocumento')}</label>
        <select {...sel(tipo, setTipo)}>
          {TIPOS_EDIT.map(t => <option key={t} value={t}>{t}</option>)}
        </select>

        <label style={{ color: C.textS, fontSize: 12, display: 'block', marginTop: 14 }}>{t('clientes.cliente')}</label>
        <select {...sel(cliId, id => { setCliId(id); setProcId('') })}>
          <option value="">{t('documentos.sinAsignar')}</option>
          {clientes.map(c => <option key={c.id} value={c.id}>{c.nombre}</option>)}
        </select>

        <label style={{ color: C.textS, fontSize: 12, display: 'block', marginTop: 14 }}>{t('clientes.expedientes')}</label>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 24, justifyContent: 'flex-end' }}>
          <button onClick={onClose} style={btn(C.border, { color: C.textS })}>{t('documentos.cancelar')}</button>
          <button onClick={handleSave} disabled={saving} style={btn(C.gold)}>
            {saving ? t('documentos.guardando') : t('documentos.guardarCambios')}
          </button>
        </div>
      </div>
    </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          <a href={`${documentosService.getDownloadUrl(doc.id)}?token=${token}`}
            download={doc.nombre} style={btn(C.gold, { textDecoration: 'none' })}>
            <Download size={13} /> {t('documentos.descargar')}
          </a>
...
export const Documentos = () => {
  ...
  const fetchDocs = async () => {
    setLoading(true)
    try { const { data } = await documentosService.getAll(); setDocs(data) }
    catch { setError(t('documentos.errorCargando')) } finally { setLoading(false) }
  }

  const handleFile = async (file) => {
    if (!file || file.type !== 'application/pdf') {
      setError(t('documentos.soloPdf')); return
    }
    ...
    try {
      ...
      setError(err.response?.data?.error || t('documentos.errorSubiendo'))
    } finally { setUploading(false) }
  }

  const handleDelete = async (id, e) => {
    e.stopPropagation()
    if (!confirm(t('documentos.confirmarEliminar'))) return
    try {
      await documentosService.remove(id)
      setDocs(prev => prev.filter(d => d.id !== id))
      if (preview?.id === id) setPreview(null)
      if (iaResult) setIaResult(null)
    } catch { setError(t('documentos.errorEliminando')) }
  }
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          ...
          <button onClick={() => fileRef.current.click()} style={btn(C.gold, { fontSize: 13, padding: '9px 18px' })}>
            <Upload size={15} /> {t('documentos.subirPdf')}
          </button>
        </div>
        ...
        <div
          ...
          {uploading ? (
            <div style={{ color: C.gold }}>
              ...
              <div style={{ fontSize: 15 }}>{t('documentos.iaAnalizando')}</div>
              <div style={{ color: C.textM, fontSize: 13, marginTop: 4 }}>{t('documentos.iaExtrayendo')}</div>
            </div>
          ) : (
            <>
              <Upload size={28} color={C.textM} style={{ marginBottom: 10 }} />
              <div style={{ color: C.text, fontSize: 15 }}>{t('documentos.arrastrarPdf')}</div>
              <div style={{ color: C.textM, fontSize: 13, marginTop: 4 }}>{t('documentos.iaReconocera')}</div>
            </>
          )}
        </div>
        ...
        {iaResult && (
          <div style={{ ...card({ marginBottom: 20, position: 'relative' }), borderColor: C.green }}>
            ...
            <div style={{ color: C.green, marginBottom: 14, display: 'flex', alignItems: 'center', gap: 8, fontWeight: 600 }}>
              <CheckCircle size={16} />
              {t('documentos.documentoReconocido')}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14, marginBottom: 10 }}>
              {[
                [t('documentos.procedimiento'), iaResult.procedimiento, C.gold, 'monospace'],
                [t('documentos.cliente'),       iaResult.cliente,       C.text, 'inherit'],
                [t('documentos.tipo'),          iaResult.tipo,          C.text, 'inherit'],
              ].map(([k, v, col, ff]) => (
                <div key={k}>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 14, alignItems: 'center' }}>
          <div style={{ position: 'relative', flex: 1 }}>
            <Search size={14} color={C.textM} style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)' }} />
            ...
            {TIPOS.map(tp => (
              <button key={tp} onClick={() => setFilter(tp)}
                style={{ padding: '6px 12px', borderRadius: 20, fontSize: 11, cursor: 'pointer',
                  border: `1px solid ${filterTipo === tp ? C.gold : C.border}`,
                  background: filterTipo === tp ? C.gold + '22' : 'transparent',
                  color: filterTipo === tp ? C.gold : C.textS }}>
                {tp}
              </button>
            ))}
          </div>
        ...
        <div style={card({ padding: 0, overflow: 'hidden' })}>
          <div style={{ display: 'grid', gridTemplateColumns: '3fr 1.5fr 1fr 70px 80px 72px',
            ...
            <div>{t('documentos.documento')}</div>
            <div>{t('documentos.clienteProcedimiento')}</div>
            <div>{t('documentos.tipo')}</div>
            <div>{t('documentos.tamano')}</div>
            <div>{t('documentos.fecha')}</div>
            <div></div>
          </div>

          {loading ? (
            <div style={{ padding: 40, textAlign: 'center', color: C.textS }}>{t('documentos.cargando')}</div>
          ) : filtered.length === 0 ? (
            <div style={{ padding: 40, textAlign: 'center', color: C.textS }}>
              {search || filterTipo !== 'Todos' ? t('documentos.noDocumentosFiltro') : t('documentos.noDocumentos')}
            </div>
          ) : filtered.map((d, i) => {
            ...
            return (
              <div key={d.id} onClick={() => setPreview(isActive ? null : d)}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <FileText size={15} color={C.gold} style={{ flexShrink: 0 }} />
                ...
                <div style={{ display: 'flex', gap: 4 }} onClick={e => e.stopPropagation()}>
                  <button title={t('clientes.editar')} onClick={() => setEditing(d)}
                    style={{ background: 'none', border: 'none', color: C.textM, cursor: 'pointer', padding: 4, borderRadius: 4 }}
                    onMouseEnter={e => e.currentTarget.style.color = C.gold}
                    onMouseLeave={e => e.currentTarget.style.color = C.textM}>
                    <Pencil size={13} />
                  </button>
                  <button title="Preview" onClick={() => setPreview(isActive ? null : d)}
                  ...
                  <button title={t('clientes.eliminar')} onClick={(e) => handleDelete(d.id, e)}
                    style={{ background: 'none', border: 'none', color: C.textM, cursor: 'pointer', padding: 4, borderRadius: 4 }}
        ...
        <div style={{ color: C.textM, fontSize: 12, marginTop: 10 }}>
          {filtered.length !== 1 ? t('documentos.documentosCountPlural', { n: filtered.length }) : t('documentos.documentosCount', { n: filtered.length })}
        </div>
      </div>

      {preview && <PreviewPanel doc={preview} onClose={() => setPreview(null)} />}
      {editing && <EditModal doc={editing} clientes={clientes} onSave={handleSaveEdit} onClose={() => setEditing(null)} />}
    </div>
  )
}
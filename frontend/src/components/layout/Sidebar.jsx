...
import { Home, Users, FileText, Mail, Calendar, Globe,
    ...
    <div style={{ position: 'fixed', top: 0, left: 225, bottom: 0, width: 340,
      ...
      <div style={{ padding: '14px 16px', borderTop: `1px solid ${C.border}` }}>
        <button onClick={onVerAgenda}
          ...
          {t('agenda.verAgenda') ?? 'Ver agenda completa'} <ChevronRight size={14} />
        </button>
      </div>
    </div>
  )
...
export const Sidebar = ({ active, setActive, isDark, onToggleTheme }) => {
  ...
  const { t, i18n } = useTranslation()           // ← aquí, dentro del componente
  ...
  const toggleLang = () => {
    ...
    localStorage.setItem('herion_lang', next)
  }

  // Items con labels traducidas
  const ITEMS = [
    { id: 'dashboard',   icon: Home,         label: t('nav.dashboard')   },
    { id: 'clientes',    icon: Users,        label: t('nav.clientes')    },
    { id: 'documentos',  icon: FileText,     label: t('nav.documentos')  },
    { id: 'correo',      icon: Mail,         label: t('nav.correo')      },
    { id: 'agenda',      icon: Calendar,     label: t('nav.agenda')      },
    { id: 'lexnet',      icon: Globe,        label: t('nav.lexnet')      },
    { id: 'facturacion', icon: () => <span style={{ fontSize: 15, fontWeight: 700, width: 15, display: 'inline-flex', justifyContent: 'center' }}>€</span>, label: t('nav.facturacion') },
    { id: 'whatsapp',    icon: WhatsAppIcon, label: t('nav.whatsapp')    },
  ]
        ...
        <div style={{ padding: '20px 20px 16px', borderBottom: `1px solid ${C.border}`,
          ...
          <div style={{ display: 'flex', gap: 5, alignItems: 'center' }}>
            ...
            <button onClick={toggleLang}
              ...
              style={{ background: 'none', border: `1px solid ${C.border}`, borderRadius: 6,
                color: C.textS, cursor: 'pointer', padding: '4px 6px',
                fontSize: 10, fontWeight: 700, letterSpacing: 0.5, transition: 'all 0.15s' }}
              onMouseEnter={e => { e.currentTarget.style.borderColor = C.gold; e.currentTarget.style.color = C.gold; e.currentTarget.style.boxShadow = `0 0 8px ${C.gold}44` }}
              onMouseLeave={e => { e.currentTarget.style.borderColor = C.border; e.currentTarget.style.color = C.textS; e.currentTarget.style.boxShadow = 'none' }}>
        ...
        {user && (
          <div style={{ padding: '14px 16px', borderTop: `1px solid ${C.border}` }}>
            ...
            <div onClick={() => dispatch(logout())}
              ...
              <LogOut size={13} /> {t('nav.cerrarSesion')}
            </div>
          </div>
        )}
      </div>

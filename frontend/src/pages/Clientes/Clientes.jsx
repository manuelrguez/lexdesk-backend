...
import { Plus, Search, Phone, Mail, MapPin, FileText, X } from 'lucide-react'
...
export const Clientes = () => {
  ...
  const fetchClientes = async () => {
    ...
    catch { setError('Error cargando clientes') } finally { setLoading(false) }
  }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setShowForm(false)
    } catch { setError('Error guardando cliente') } finally { setSaving(false) }
  }

  const handleDelete = async (id) => {
    if (!confirm('¿Eliminar este cliente?')) return
    try { await clientesService.remove(id); setClientes(prev => prev.filter(c => c.id !== id)); if (selected?.id === id) setSelected(null) }
    catch { setError('Error eliminando cliente') }
  }
  ...
  const handleSaveProc = async () => {
    ...
    } catch { setError('Error guardando procedimiento') } finally { setSavingProc(false) }
  }
  ...
  const handleDeleteProc = async (procId) => {
    ...
    try {
      ...
      setSelected({ ...selected, procedimientos: upd })
      setClientes(prev => prev.map(c => c.id === selected.id ? { ...c, procedimientos: upd } : c))
    } catch { setError('Error eliminando procedimiento') }
  }
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div style={{ fontFamily: font.display, fontSize: 28, color: C.text, fontWeight: 600 }}>Clientes</div>
        <button onClick={openNew} style={btn(C.gold)}><Plus size={15} /> Nuevo cliente</button>
      </div>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={card({ padding: 0, overflow: 'hidden' })}>
            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1.5fr 1fr 80px',
              ...
              <div>Cliente</div><div>Contacto</div><div>Expedientes</div><div></div>
            </div>
            ...
            {loading ? (
              ...
              <div style={{ padding: 40, textAlign: 'center', color: C.textS }}>
                {search ? 'No se encontraron clientes' : 'No hay clientes. Crea el primero →'}
              </div>
            ) : filtered.map((c, i) => (
            ...
            <div style={card({ marginBottom: 12 })}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
                <div>
                  ...
                  <div style={{ color: C.textM, fontSize: 14, marginTop: 3 }}>{selected.nif || 'Sin NIF'}</div>
                </div>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
                <button onClick={() => openEdit(selected)} style={{ ...btn(C.gold), flex: 1, justifyContent: 'center' }}>Editar</button>
            ...
            <div style={card()}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
                <div style={{ color: C.textS, fontSize: 13, textTransform: 'uppercase', letterSpacing: 1 }}>
                  Expedientes ({selected.procedimientos?.length || 0})
                </div>
                <button onClick={openNewProc}
                  style={{ ...btn(C.gold, { fontSize: 12, padding: '5px 12px' }) }}>
                  <Plus size={12} /> Nuevo
                </button>
              </div>

              {(!selected.procedimientos || selected.procedimientos.length === 0) ? (
                <div style={{ color: C.textM, fontSize: 14 }}>Sin expedientes registrados</div>
      ...
      {showForm && (
        <div style={{ position: 'fixed', inset: 0, background: '#000000aa', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: C.sidebar, border: `1px solid ${C.border}`, borderRadius: 16, padding: 32, width: 500 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
              <span style={{ fontFamily: font.display, fontSize: 22, color: C.text }}>{editing ? 'Editar cliente' : 'Nuevo cliente'}</span>
              <button onClick={() => setShowForm(false)} style={{ background: 'none', border: 'none', color: C.textS, cursor: 'pointer' }}><X size={18} /></button>
            </div>
            {[
              { key: 'nombre',    label: 'Nombre / Razón social *', placeholder: 'Empresa S.L.' },
              { key: 'nif',       label: 'NIF / CIF',               placeholder: 'B12345678' },
              { key: 'telefono',  label: 'Teléfono',                placeholder: '911 234 567' },
              { key: 'email',     label: 'Email',                   placeholder: 'contacto@empresa.es' },
              { key: 'direccion', label: 'Dirección',               placeholder: 'Calle Mayor 1, 28001 Madrid' },
      ...
      {showProcForm && (
        <div style={{ position: 'fixed', inset: 0, background: '#000000aa', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: C.sidebar, border: `1px solid ${C.border}`, borderRadius: 16, padding: 32, width: 480 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
              <span style={{ fontFamily: font.display, fontSize: 22, color: C.text }}>{editingProc ? 'Editar expediente' : 'Nuevo expediente'}</span>
              <button onClick={() => setShowProcForm(false)} style={{ background: 'none', border: 'none', color: C.textS, cursor: 'pointer' }}><X size={18} /></button>
            </div>
            {[
              { key: 'numero',  label: 'Número de procedimiento *', placeholder: '2024/001234' },
              { key: 'tipo',    label: 'Tipo',                      placeholder: 'Mercantil, Civil, Penal...' },
              { key: 'juzgado', label: 'Juzgado',                   placeholder: 'Juzgado Mercantil nº 3 Madrid' },
            ].map(({ key, label, placeholder }) => (
              <div key={key} style={{ marginBottom: 14 }}>
                <Lbl>{label}</Lbl>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={handleSaveProc} disabled={savingProc || !procForm.numero.trim()}
                ...
                {savingProc ? 'Guardando...' : editingProc ? 'Guardar cambios' : 'Crear expediente'}
              </button>
